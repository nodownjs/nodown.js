...
describe("Alert", () => {
  it("Without title", () => {
    ...
    const alertResult = `<div class="alert question"><p>${alertText}</p></div>`;
  ...
  it("Without title and multi lines", () => {
    const alertTest = `>? ${alertText}\n>? ${alertText}`;
    const alertResult = `<div class="alert question"><p>${alertText}</p><p>${alertText}</p></div>`;
    const alert = generateTest(alertTest);
    expect(alert).toBe(alertResult);
  });
  ...
  it("Warning alert multi lines", () => {
    const alertTest = `>! # ${alertTitle}\n>! ${alertText}\n>! ${alertText}`;
    const alertResult = `<div class="alert warning"><h4>${alertTitle}</h4><p>${alertText}</p><p>${alertText}</p></div>`;
    const alert = generateTest(alertTest);
    expect(alert).toBe(alertResult);
  });
